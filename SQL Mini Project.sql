/*SQL MINI PROJECT
This is subject to review, all opinions and questions are welcome */
...
SELECT TOP 10
    ...
    a.CountryRegion AS ShippedCountry,
    a.City AS ShippedCity,
...
GROUP BY c.CustomerID, c.FirstName, c.LastName, a.CountryRegion, a.City
...
SELECT c.CustomerID, c.CompanyName, SUM(sod.OrderQty * sod.UnitPrice) AS Revenue,
    CASE 
        WHEN SUM(sod.OrderQty * sod.UnitPrice) >= 10000 THEN 'High Revenue'
        WHEN SUM(sod.OrderQty * sod.UnitPrice) >= 5000 THEN 'Medium Revenue'
        WHEN SUM(sod.OrderQty * sod.UnitPrice) >= 1000 THEN 'Low Revenue'
...
/*Question 5:
What are the top 3 selling products (include product name) in each category (include category name) 
